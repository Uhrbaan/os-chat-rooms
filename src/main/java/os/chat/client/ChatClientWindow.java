...
/*
 * This Class is a graphical interface for your chat client. It must not be modified. Only a
 * ChatClient object implementing the CommandsFromWindow interface is required. All the logic and
 * client-server interface goes in this ChatClient object. (Feel free to observe the code of this
 * class if you are curious about how one can create graphical interfaces.)
 */
...
public class ChatClientWindow implements CommandsToWindow {
	...
	public ChatClientWindow() {
		...
		String userName = (String) JOptionPane.showInputDialog(null, "Enter your name:\n",
				"RMI-chat", JOptionPane.PLAIN_MESSAGE, null, null, "");
		...
		btnSend.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (selectedTab == -1) {
					...
					JOptionPane.showMessageDialog(frmChatClient,
							"You must join a chat room before sending messages!", "Error message",
		...
		btnJoinRoom.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (index == -1) {
					...
					JOptionPane.showMessageDialog(frmChatClient,
							"You must select a room name from the list first!", "Error message",
							JOptionPane.ERROR_MESSAGE);
				} else {
					...
					if (chats.containsKey(r)) {
						JOptionPane.showMessageDialog(frmChatClient,
								"You already joined this chat room.", "Error message",
								JOptionPane.ERROR_MESSAGE);
					} else {
						...
						if (!status) {
							...
							JOptionPane.showMessageDialog(frmChatClient, "Joining chat room " + r
									+ " was not successful.\nCheck client/server interaction.",
							...
							jta.setText("You have now joined the discussion on " + r
									+ " and can now send and receive messages!\n");
							// embed it in a scroll pane
							JScrollPane jsc = new JScrollPane(jta);
							// disable horizontal scrolling
							jsc.setHorizontalScrollBarPolicy(
		...
		btnLeaveRoom.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (selectedTab == -1) {
					JOptionPane.showMessageDialog(frmChatClient, "There is no chat room to leave.",
							"Error message", JOptionPane.ERROR_MESSAGE);
				} else {
		...
		btnCreateRoom.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (roomName.equalsIgnoreCase("")) {
					...
					JOptionPane.showMessageDialog(frmChatClient,
							"No Chat Room name to create given!", "Error message",
							JOptionPane.ERROR_MESSAGE);
				} else {
					// create the room
					...
					if (!success) {
						JOptionPane.showMessageDialog(frmChatClient,
								"Creating Chat room " + roomName + " was not successful.",
								"Error message", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
	...
	public void publish(String chatName, String message) {
		...
		if (!chats.containsKey(chatName)) {
			JOptionPane.showMessageDialog(frmChatClient,
					"The client asks for sending a message\nto a non-existing chat room ("
							+ chatName + ").",
					"Error message", JOptionPane.ERROR_MESSAGE);
		} else {
